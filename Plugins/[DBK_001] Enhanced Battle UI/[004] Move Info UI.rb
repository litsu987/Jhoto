...
class Battle::Scene  
  ...
  def pbUpdateMoveInfoWindow(battler, specialAction, cw)
    ...
    when "CategoryDependsOnHigherDamageTera",
      ...
      if terastal
        ...
        when "CategoryDependsOnHigherDamageTera"
          ...
          basePower = calcPower = finalPower = 100
    ...
    textPos.push(
      ...
      [_INTL("Pod:"),   xpos + 542,  ypos + 11, :center, BASE_LIGHT, SHADOW_LIGHT],
      [_INTL("Pre:"),   xpos + 536,  ypos + 40, :center, BASE_LIGHT, SHADOW_LIGHT],
      [_INTL("Efect:"), xpos + 286,           ypos + 40, :left,   BASE_LIGHT, SHADOW_LIGHT],
      [power,           Graphics.width - 34,  ypos + 10, :center, powBase,    powShadow],
      [accuracy,        Graphics.width - 34,  ypos + 40, :center, accBase,    accShadow],
      [effect_rate,     xpos + 370, ypos + 40, :center, effBase,    effShadow]
    )
    pbAddPluginText(xpos, ypos, textPos, move, type, battler, specialAction, cw)
    pbDrawTextPositions(@enhancedUIOverlay, textPos)
    drawTextEx(@enhancedUIOverlay, xpos + 10, ypos + 72, Graphics.width - 10, 2, 
  ...
  #-----------------------------------------------------------------------------
  def pbAddPluginText(xpos, ypos, textPos, move, type, battler, specialAction, cw)
    case specialAction
    #---------------------------------------------------------------------------
    ...
    when :zmove
      ...
      if cw.mode == 2
        if move.zMove? && move.has_zpower?
          ...
          case effect
          when "HealUser"    then text = _INTL("Restaura completamente los PS del usuario.")
          when "HealSwitch"  then text = _INTL("Restaura completamente los PS del Pokémon que entra al combate.")
          when "CriticalHit" then text = _INTL("Sube la probabilidad de ataques críticos del usuario.")
          when "ResetStats"  then text = _INTL("Reinicia las características reducidas del usuario.")
          when "FollowMe"    then text = _INTL("El usuario se vuelve el centro de atención.")
          else
            if stage
              stat = (effect == "AllStats") ? "stats" : GameData::Stat.get(effect.to_sym).name
              case stage
              when "3" then text = _INTL("Aumentó drásticamente su {1}.")
              when "2" then text = _INTL("Aumentó drásticamente su {1}.", stat)
              else          text = _INTL("Aumentó su {1}.", stat)
              end
            end
          end
          textPos.push([_INTL("Poder Z: #{text}"), xpos + 10, ypos + 128, :left, BASE_RAISED, SHADOW_RAISED]) if text
        end
      end
    #---------------------------------------------------------------------------
    ...
    when :tera
      ...
      if cw.teraType > 0 && battler.typeTeraBoosted?(type, true)
        textPos.push([_INTL("Teratipo: Ataque potenciado por la Teracristalización."), xpos + 10, ypos + 128, :left, BASE_RAISED, SHADOW_RAISED])
      end
    end
  end
end