...
class VoltorbFlip
  ...
  def pbNewGame
    ...
    pbDrawShadowText(@sprites["text"].bitmap, 8, 22, 118, 26,
                     _INTL("Tus monedas"), Color.new(60, 60, 60), Color.new(150, 190, 170), 1)
    pbDrawShadowText(@sprites["text"].bitmap, 8, 88, 118, 26,
                     _INTL("Monedas premio"), Color.new(60, 60, 60), Color.new(150, 190, 170), 1)
    # Draw current level
        ...
        @squares.length.times do |i|
          if (@index[0] * 64) + 128 == @squares[i][0] && @index[1] * 64 == @squares[i][1] && @squares[i][3] == false
            ...
            if @squares[i][2] == 0
              ...
              pbMessage("\\me[Voltorb Flip game over]" + _INTL("¡Oh no! ¡Has obtenido 0 monedas!") + "\\wtnp[80]")
              pbShowAndDispose
              @sprites["mark"].bitmap.clear
              if @level > 1
