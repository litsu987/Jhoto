  ...
  def refresh
    ...
    if @refreshBitmap
      ...
      if @isContinue
        ...
        textpos.push([_INTL("Medallas:"), 102, 74, :left, TEXT_COLOR, TEXT_SHADOW_COLOR])
        textpos.push([@trainer.badge_count.to_s, 280, 74, :right, TEXT_COLOR, TEXT_SHADOW_COLOR])
        textpos.push([_INTL("Pokédex:"), 102, 113, :left, TEXT_COLOR, TEXT_SHADOW_COLOR])
        textpos.push([@trainer.pokedex.seen_count.to_s, 282, 113, :right, TEXT_COLOR, TEXT_SHADOW_COLOR])
        textpos.push([_INTL("Tiempo:"), 102, 154, :left, TEXT_COLOR, TEXT_SHADOW_COLOR])
        hour = @totalsec / 60 / 60
        min  = @totalsec / 60 % 60
        if hour > 0
          textpos.push([_INTL("{1}h {2}m", hour, min), 278, 154, :right, TEXT_COLOR, TEXT_SHADOW_COLOR])
        else
          textpos.push([_INTL("{1}m", min),278, 154, :right, TEXT_COLOR, TEXT_SHADOW_COLOR])
        end
        if @trainer.male?
          textpos.push([@trainer.name, 100, 42, :left, MALE_TEXT_COLOR, MALE_TEXT_SHADOW_COLOR])
        elsif @trainer.female?
          textpos.push([@trainer.name, 100, 42, :left, FEMALE_TEXT_COLOR, FEMALE_TEXT_SHADOW_COLOR])
        else
          textpos.push([@trainer.name, 100, 42, :left, TEXT_COLOR, TEXT_SHADOW_COLOR])
        end
...
class PokemonLoad_Scene
  def pbStartScene(commands, show_continue, trainer, stats, map_id)
    ...
    commands.length.times do |i|
      ...
      @sprites["panel#{i}"].x = 108
      @sprites["panel#{i}"].y = y
      @sprites["panel#{i}"].pbRefresh
      y += (show_continue && i == 0) ? 218 : 48
    end
...
=begin
    ...
    if meta
      ...
      @sprites["player"].x = 166 - (charwidth / 8)
      @sprites["player"].y = 150 - (charheight / 8)
      @sprites["player"].z = 99999
    end
    trainer.party.each_with_index do |pkmn, i|
      @sprites["party#{i}"] = PokemonIconSprite.new(pkmn, @viewport)
      @sprites["party#{i}"].setOffset(PictureOrigin::CENTER)
      @sprites["party#{i}"].x = 422 + (66 * (i % 2))
      @sprites["party#{i}"].y = 94 + (50 * (i / 2))
      @sprites["party#{i}"].z = 99999
    end
=end
    # Mostrar icono PSS en el menú load (créditos a WolfPP)
    @sprites["pssicon"]=IconSprite.new(116,87,@viewport)
    @sprites["pssicon"].setBitmap("Graphics/Pictures/IconoChico") if trainer.gender==0
    @sprites["pssicon"].setBitmap("Graphics/Pictures/IconoChica") if trainer.gender==1
    
    @sprites["pssicon"].zoom_x = 0.60
    @sprites["pssicon"].zoom_y = 0.60
  end
