...
@sprites["gendergirl"].x = Graphics.width - 50
...
#===================================================
  ...
  @sprites["girl"].x = 500  # Cambia 512 al valor deseado
...
@sprites["boy"].zoom_x += 0.0005 * 2 if @sprites["boy"].zoom_x != 2.0
@sprites["boy"].zoom_y += 0.0005 * 2 if @sprites["boy"].zoom_y != 2.0
...
@sprites["girl"].zoom_x -= 0.0005 * 2
@sprites["girl"].zoom_y -= 0.0005 * 2
...
@sprites["girl"].zoom_x += 0.0005 * 2 if @sprites["girl"].zoom_x != 2.0
@sprites["girl"].zoom_y += 0.0005 * 2 if @sprites["girl"].zoom_y != 2.0
...
@sprites["boy"].zoom_x -= 0.0005 * 2
@sprites["boy"].zoom_y -= 0.0005 * 2
...
if !pbConfirmMessage(_INTL("Eres un chico, verdad?"))
22.times do
Graphics.update
@sprites["gendergirl"].x -= 16
@sprites["girl"].x -= 12
...
@sprites["msgwindow"].visible = true
  ...
  pbMessage("Me gustaría saber tu nombre.\nPor favor, dímelo.")
  selectName
end
...
if !pbConfirmMessage(_INTL("Eres una chica, verdad?"))
22.times do
Graphics.update
@sprites["genderboy"].x += 16
@sprites["boy"].x += 12
...
if WRITETRAINERNAME
  pbHide
  pbTrainerName
  if !pbConfirmMessage(_INTL("Tu nombre es {1}?", $player.name))
    selectName
  else
    @finished = true
  end
