...
class PokemonSummary_Scene
  def pbStartScene(party, partyindex, inbattle = false, page=1)
      ...
      @sprites["pokemon"].x = 134
      @sprites["pokemon"].y = 236
      ...
      @sprites["itemicon"] = ItemIconSprite.new(37, 365, @pokemon.item_id, @viewport)
  ...
  def drawPageOneEgg
      ...
      # Draw all text
      drawFormattedTextEx(overlay, 310, 72, 268, memo)
  ...
  def drawPageFourSelecting(move_to_learn)
      ...
      # Write various bits of text
      textpos = [
        [_INTL("MOVIMIENTOS"), 20, 28, :left, base, shadow],
        [_INTL("CATEGORÍA"), 10, 128, :left, base, shadow],
        [_INTL("POTENCIA"), 10, 160, :left, base, shadow],
        [_INTL("PRECISIÓN"), 10, 192, :left, base, shadow],
      ...
      limit.times do |i|
        ...
        if move
          ...
          imagepos.push([_INTL("Graphics/UI/types"), 344, yPos - 8, 0, type_number * 28, 64, 28])
          textpos.push([move.name, 410, yPos, :left, moveBase, moveShadow])
          if move.total_pp > 0
            textpos.push([_INTL("PP"), 410, yPos + 30, :left, moveBase, moveShadow])
            ppfraction = 0
            if move.pp == 0
              ppfraction = 3
            ...
            textpos.push([sprintf("%d/%d", move.pp, move.total_pp), 510, yPos + 30, :right,
                          ppBase[ppfraction], ppShadow[ppfraction]])
          end
        else
          textpos.push(["-", 440, yPos, :left, moveBase, moveShadow])
          textpos.push(["--", 500, yPos + 4, :right, moveBase, moveShadow])
        end
        yPos += 84
    ...
    def pbChooseMoveToForget(move_to_learn)
      ...
      def showAbilityDescription(pokemon)
        ...
        imagepos=[]
        ballimage = sprintf("Graphics/UI/Summary/icon_ball_%s", @pokemon.poke_ball)
        imagepos.push([ballimage,12,70,0,0,-1,-1])
        pbDrawImagePositions(overlay,imagepos)
        base=Color.new(248,248,248)
        ...
        textpos=[
           ...
           [pokename,48,78,0,base,shadow2],
           [pokemon.level.to_s,54, 114, 0, Color.new(255, 255, 255), Color.new(255, 176, 176)],
           [_INTL("Habilidad:"),238,22,0,base,shadow2],
           [abilityname,342,22,0,base,shadow2],
           [_INTL("Objeto"), 86, 368, 0, base, shadow2]
          ] 
        if @pokemon.hasItem?
          textpos.push([@pokemon.item.name, 16, 410, 0, Color.new(64, 64, 64), Color.new(176, 176, 176)])
        else
          textpos.push([_INTL("Ninguno"), 16, 410, 0, Color.new(192, 200, 208), Color.new(208, 216, 224)])
        end
        if @pokemon.male?
          textpos.push([_INTL("♂"), 10, 120, 0, Color.new(24, 112, 216), Color.new(136, 168, 208)])
        elsif @pokemon.female?
          textpos.push([_INTL("♀"), 10, 120, 0, Color.new(248, 56, 32), Color.new(224, 152, 144)])
        end
        # Draw all text
        pbDrawTextPositions(overlay, textpos)
        # Draw the Pokémon's markings
        drawMarkings(overlay, 84, 292)
        pbDrawTextPositions(overlay,textpos)
        drawTextEx(overlay,289,95,230,10,abilitydesc,Color.new(64,64,64),shadow)  
        loop do
          Graphics.update
          Input.update
          pbUpdate
