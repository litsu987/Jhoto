...
class PokemonBagPartyPanel < Sprite
  ...
  def initialize(pokemon, index, viewport=nil)
    ...
    @refreshing = true
    self.x = (index % 2) * 144 + 16
    self.y = 106 * (index / 2) + 12
...
class PokemonBag_Scene
  ...
  def pbUpdate
    ...
    @sprites["panorama"].x  = 2 if @sprites["panorama"].x == - 56
  ...
  def pbStartScene(bag, party = $player.party, choosing = false, filterproc = nil, resetpocket = true)
    ...
    @sprites["currentpocket"].src_rect = Rect.new(50, 0, 28, 28)
    
    @sprites["itemlist"] = Window_PokemonBag.new(@bag, @filterlist, lastpocket, 264, 60, 376, 70 + ITEMSVISIBLE * 32)
    ...
    @sprites["itemicon"] = ItemIconSprite.new(60, Graphics.height - 55, nil, @viewport)
    @sprites["itemtext"] = Window_UnformattedTextPokemon.newWithSize(
      "", 85, 328, Graphics.width - 72 - 24, 128, @viewport
    )
  ...
  def pbRefreshIndexChanged
    ...
    pbDrawTextPositions(
      overlay,
      [[PokemonBag.pocket_names[@bag.last_viewed_pocket - 1], 575, 20, :center, POCKETNAMEBASECOLOR, POCKETNAMEOUTLINECOLOR, true, Graphics.width]]
    )
    # Draw slider arrows
    showslider = false
