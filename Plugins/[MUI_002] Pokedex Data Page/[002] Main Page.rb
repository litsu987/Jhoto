...
class PokemonPokedexInfo_Scene 
  ...
  def drawPageData
    ...
    imagepos.push([path + "habitats", 520, 194, 0, 48 * habitat, 64, 48])
    #---------------------------------------------------------------------------
    # Draws body shape icon.
    #---------------------------------------------------------------------------
    shape = GameData::BodyShape.get(species_data.shape).icon_position
    imagepos.push(["Graphics/UI/Pokedex/icon_shapes", 445, 186, 0, 60 * shape, 60, 60])
    #---------------------------------------------------------------------------
    ...
    egg_groups.each_with_index do |group, i|
      ...
      group_y = (egg_groups.length == 1) ? 188 : 188 + 30 * i
      imagepos.push([path + "egg_groups", 364, group_y, rectX, 28 * rectY, 62, 28])
    end
    #---------------------------------------------------------------------------
    # Draws the base stats text and bars.
    #---------------------------------------------------------------------------
    textpos.push(
      [_INTL("PS"),        10, 108, :left, base, shadow, :outline],
      [_INTL("Ataque"),    10, 136, :left, base, shadow, :outline],
      [_INTL("Defensa"),   10, 164, :left, base, shadow, :outline],
      [_INTL("At. Esp."),  10, 192, :left, base, shadow, :outline],
      [_INTL("Df. Esp."),  10, 220, :left, base, shadow, :outline],
      [_INTL("Velocid."),  10, 248, :left, base, shadow, :outline]
    )
    ...
    #---------------------------------------------------------------------------
    textpos.push([_INTL("Habilid."), 368, 265, :center, base, shadow, :outline],
                 [_INTL("Movimien."),497, 265, :center, base, shadow, :outline])			  
    #---------------------------------------------------------------------------
    ...
    if special_form
      ...
      if prevo == species_data.species || @data_hash[:family].empty?
        imagepos.push([path + "evolutions", 294, ICONS_POS_Y - 34, 0, 0, 272, 64])
        @sprites["familyicon0"].pbSetParams(@species, @gender, @form)
      ...
      #-------------------------------------------------------------------------
        ...
        imagepos.push([path + "evolutions", 294, ICONS_POS_Y - 34, 0, 64 * stages, 272, 64])
        @sprites["familyicon0"].pbSetParams(@species, @gender, @form)
