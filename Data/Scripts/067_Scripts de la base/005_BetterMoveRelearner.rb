...
class MoveRelearner_Scene
  def pbStartScene(pokemon, moves)
      ...
      @sprites["pokeicon"].x = 400
      @sprites["pokeicon"].y = 96
    ...
    def pbDrawMoveList
      ...
      @pokemon.types.each_with_index do |type, i|
        ...
        type_x = (@pokemon.types.length == 1) ? 520 : 450 + (70 * i)
        overlay.blt(type_x, 70, @typebitmap.bitmap, type_rect)
      end
      textpos = [
        [_INTL("¿Enseñar qué movimiento?"), 12, 26, :left, Color.new(255, 255, 255), Color.new(64, 64, 64)]
      ]
      imagepos = []
      yPos = 88
      VISIBLEMOVES.times do |i|
        ...
        if moveobject
          ...
          imagepos.push([_INTL("Graphics/UI/types"), 25, yPos - 3, 0, type_number * 28, 64, 28])
          textpos.push([moveData.name, 90, yPos + 6, :left, Color.new(248, 248, 248), Color.black])
          textpos.push([moveobject[1], 12, yPos+32, :left, Color.new(248, 248, 248), Color.black]) if moveobject[1]
          textpos.push([_INTL("PP"), 130, yPos + 32, :left, Color.new(255, 255, 255), Color.new(64, 64, 64)])
          if moveData.total_pp > 0
            textpos.push([moveData.total_pp.to_s + "/" + moveData.total_pp.to_s, 230, yPos + 32, :right,
                          Color.new(255, 255, 255), Color.new(64, 64, 64)])
          else
            textpos.push(["--", 220, yPos + 32, :right, Color.new(64, 64, 64), Color.new(176, 176, 176)])
          end
        end
        yPos += 64
      end
      ...
      textpos.push([_INTL("CATEGORÍA"), 364, 130, :left, Color.new(248, 248, 248), Color.black])
      textpos.push([_INTL("PODER"), 364, 160, :left, Color.new(248, 248, 248), Color.black])
      textpos.push([power <= 1 ? power == 1 ? "???" : "---" : power.to_s, 580, 160, :center,
                    Color.new(64, 64, 64), Color.new(176, 176, 176)])
      textpos.push([_INTL("PRECISIÓN"), 364, 194, :left, Color.new(248, 248, 248), Color.black])
      textpos.push([accuracy == 0 ? "---" : "#{accuracy}%", 584, 192, :center,
                    Color.new(64, 64, 64), Color.new(176, 176, 176)])
      pbDrawTextPositions(overlay, textpos)
      imagepos.push(["Graphics/UI/category", 552, 122, 0, category * 28, 64, 28])
      ...
      drawTextEx(overlay, 372, 221, 230, 5, selMoveData.description,
                 Color.new(64, 64, 64), Color.new(176, 176, 176))
    end
end
