...
class BadgeCase_Scene
  ...
  def pbStartScene
    ...
    @sprites["leadersprite"] = IconSprite.new(320,241,@viewport)
    @sprites["acepokemon"] = PokemonSpeciesIconSprite.new(nil,@viewport)
    @sprites["acepokemon"].setOffset(PictureOrigin::CENTER)
    @sprites["acepokemon"].x = 45
    @sprites["acepokemon"].y = 358
    @sprites["badge"] = IconSprite.new(30,126,@viewport)
  ...
  def drawBadgePage
    ...
    if $PokemonGlobal.badges.has?(@badges[@badgeindex].id)
      ...
      @sprites["leadersprite"].zoom_x = 288 / @sprites["leadersprite"].src_rect.width.to_f
    ...
    textpos = [
      [_INTL("INFORMACIÓN"), 26, 25, :left, base, shadow],
      [_INTL("Obtenido"), 316, 98, :left, base, shadow],
      [_INTL("Tipo"), 336, 132, :left, base, shadow],
      [_INTL("Ubicación"), 316, 170, :left, base, shadow],
      [_INTL("Líder"), 336, 208, :left, base, shadow],
      [_INTL("POKEMON"), 120, 350, :left, base, shadow],
      [_INTL("INSIGNIA"), 127, 368, :left, base, shadow],
    ]
    if $PokemonGlobal.badges.has?(@badges[@badgeindex].id) || BADGE_NAME_ALWAYS
      textpos.push([@badges[@badgeindex].name, 26, 76, :left, base, shadow])
    else
      textpos.push([_INTL("???"), 156, 148, :left, base, shadow])
    end
    if $PokemonGlobal.badges.has?(@badges[@badgeindex].id) || BADGE_LOCATION_ALWAYS
      textpos.push([@badges[@badgeindex].location, 532, 170, :center, Color.new(64, 64, 64), Color.new(176, 176, 176)])
    else
      textpos.push([_INTL("???"), 425, 150, :center, Color.new(64, 64, 64), Color.new(176, 176, 176)])
    end
    if $PokemonGlobal.badges.has?(@badges[@badgeindex].id)
      textpos.push([@badges[@badgeindex].leadername, 530, 208, :center, Color.new(64, 64, 64), Color.new(176, 176, 176)])
      textpos.push([@badges[@badgeindex].acepokemon.to_s.capitalize, 80, 406, :left, Color.new(64, 64, 64), Color.new(176, 176, 176)])
      time = $PokemonGlobal.badges.get_time(@badges[@badgeindex].id)
      textpos.push([_INTL("{1} {2} {3}",time.day,pbGetMonthName(time.mon),time.year), 530, 98, :center, Color.new(64, 64, 64), Color.new(176, 176, 176)])
    else
    ...
    if $PokemonGlobal.badges.has?(@badges[@badgeindex].id) || BADGE_TYPE_ALWAYS
      ...
      overlay.blt(500, 132, @typebitmap.bitmap, type_rect)
    else
