...
class Battle::Scene
  ...
  def pbUpdateBallSelection(items, index, showDesc = false)
    ...
    textPos = [
      [_INTL("C: Usar"), 32 + 4, textY + 21, :center, BASE_LIGHT],
      [action, Graphics.width - 590, textY - 1, :center, BASE_LIGHT],
      [name, Graphics.width / 2 - 66, textY + 5, :center, BASE_LIGHT, SHADOW_LIGHT]
    ]
    ...
    range.each_with_index do |pos, i|
      if pos < 0 || pos > items.length - 1
        ...
        if try_item
          ...
          textPos.push([items[pos][1].to_s, x - 1, ballY + 2, :center, BASE_LIGHT, SHADOW_LIGHT])
        end
      end
    end
    pbDrawImagePositions(@enhancedUIOverlay, imagePos)
    pbDrawTextPositions(@enhancedUIOverlay, textPos)
    drawTextEx(@enhancedUIOverlay, 8, ypos - 16, Graphics.width - 10, 2, 
