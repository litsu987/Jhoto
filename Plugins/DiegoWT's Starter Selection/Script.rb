...
class DiegoWTsStarterSelection
  ...
  def pbOpenScene
    ...
    @sprites["textbox"].text = _INTL("<ac>Elige un Pokémon.</ac>")
    pbStartChoosing
  end
    ...
    if switch == 0
      ...
      if StarterSelSettings::INSTYLE == 2 # Checks for interface style
        pbDrawTextPositions(@overlay,[
          ["SÍ",388,190,0,Color.new(255,255,255),Color.new(140,140,140),false,Graphics.width],
          ["NO",388,236,0,Color.new(255,255,255),Color.new(140,140,140),false,Graphics.width]])
      else
        pbDrawTextPositions(@overlay,[
          ["SÍ",442,202,2,Color.new(255,255,255),Color.new(74,74,74),true,Graphics.width],
  ...
  def pbChooseBall
    ...
    if @pokemon.types[1] != @pokemon.types[0] && StarterSelSettings::TYPE2COLOR && @pokemon.types[1]
      @sprites["textbox"].text = _INTL("<ac>¿Eliges a #{@pkmnname}, <br>el Pokémon tipo #{type1}/#{type2}?</ac>")
    elsif StarterSelSettings::TYPE2COLOR
      @sprites["textbox"].text = _INTL("<ac>¿Eliges a #{@pkmnname}, <br>el Pokémon tipo #{type1}?</ac>")
    else
      @sprites["textbox"].text = _INTL("<ac>¿Eliges a #{@pkmnname}, <br>el Pokémon tipo #{type1}?</ac>")
    end
