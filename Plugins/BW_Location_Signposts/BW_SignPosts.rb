...
class LocationWindow
  ...
  def initialize(name)
    ...
    if @extended_route
      textos=[
      [name,59+14,16-0,0,Color.new(255,255,255),Color.new(115,115,115)],
      ]
    else
      textos=[
      [name,59,16-0,0,Color.new(255,255,255),Color.new(115,115,115)],
      ]
    end
    ...
    if @route_number != nil
      if @route_number.to_i >= 100 #Different positions according to route number
       pbDrawRouteNumber(@route_number,@window.bitmap,32-16,14+2)
      elsif @route_number.to_i >= 10
       pbDrawRouteNumber(@route_number,@window.bitmap,32-16,14+2)
      else
       pbDrawRouteNumber(@route_number,@window.bitmap,32-6,14+2)
     end
    end
    pbDrawTextPositions(overlay,textos)
  end
