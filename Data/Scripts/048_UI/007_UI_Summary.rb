...
class MoveSelectionSprite < Sprite
  ...
  def refresh
    ...
    h = @movesel.height / 2
    self.x = 300
    self.y = 84 + (self.index * 84)
    self.y -= 86 if @fifthmove
...
class PokemonSummary_Scene
  ...
  def pbStartScene(party, partyindex, inbattle = false)
    ...
    @sprites["pokemon"].x = 154
    @sprites["pokemon"].y = 226
    ...
    @sprites["itemicon"] = ItemIconSprite.new(50, 310, @pokemon.item_id, @viewport)
  ...
  def drawPage(page)
    ...
    textpos = [
      ...
      [@pokemon.level.to_s, 46, 92, :left, Color.new(255, 255, 255), Color.new(176, 176, 176)],
    ...
    if @pokemon.male?
      textpos.push([_INTL("♂"), 10, 84, :left, Color.new(24, 146, 240), Color.new(13, 73, 119)])
    elsif @pokemon.female?
      textpos.push([_INTL("♀"), 10, 84, :left, Color.new(249, 93, 210), Color.new(128, 20, 90)])
    end
    # Draw all text
    pbDrawTextPositions(overlay, textpos)
    # Draw the Pokémon's markings
    drawMarkings(overlay, 184, 292)
  ...
  def drawPageOne
    ...
    # Write various bits of text
    textpos = [
      [_INTL("No. Dex"), 300, 100, :left, base, shadow],
      [_INTL("Especie"), 300, 136, :left, base, shadow],
      [@pokemon.speciesName, 546, 136, :center, Color.new(64, 64, 64), Color.new(176, 176, 176)],
      [_INTL("Tipo"), 298, 172, :left, base, shadow],
      [_INTL("EO"), 300, 208, :left, base, shadow],
      [_INTL("No. ID"), 300, 244, :left, base, shadow]
    ]
    ...
    if dexnum <= 0
      textpos.push(["???", 520, 106, :center, dexNumBase, dexNumShadow])
    else
      dexnum -= 1 if dexnumshift
      textpos.push([sprintf("%03d", dexnum), 544, 100, :center, dexNumBase, dexNumShadow])
    end
    # Write Original Trainer's name and ID number
    if @pokemon.owner.name.empty?
      textpos.push([_INTL("PRESTADO"), 535, 182, :center, Color.new(64, 64, 64), Color.new(176, 176, 176)])
      textpos.push(["?????", 535, 214, :center, Color.new(64, 64, 64), Color.new(176, 176, 176)])
    else
      ...
      textpos.push([@pokemon.owner.name, 544, 208, :center, ownerbase, ownershadow])
      textpos.push([sprintf("%05d", @pokemon.owner.public_id), 544, 244, :center,
    ...
    if @pokemon.shadowPokemon?
      ...
      textpos.push([_INTL("Puntos Exp."), 300, 286, :left, base, shadow])
      textpos.push([@pokemon.exp.to_s_formatted, 600, 318, :right, Color.new(64, 64, 64), Color.new(176, 176, 176)])
      textpos.push([_INTL("Siguiente Nv."), 300, 360, :left, base, shadow])
      textpos.push([(endexp - @pokemon.exp).to_s_formatted, 600, 396, :right, Color.new(64, 64, 64), Color.new(176, 176, 176)])
    end
    # Draw all text
    pbDrawTextPositions(overlay, textpos)
    # Draw Pokémon type(s)
    @pokemon.types.each_with_index do |type, i|
      ...
      type_x = (@pokemon.types.length == 1) ? 514 : 480 + (66 * i)
      overlay.blt(type_x, 168, @typebitmap.bitmap, type_rect)
    end
    # Draw Exp bar
    if @pokemon.level < GameData::GrowthRate.max_level
      ...
      pbDrawImagePositions(overlay,
                           [["Graphics/UI/Summary/overlay_exp", 453, 426, 0, 0, w, 6]])
    end
  end
  ...
  def drawPageTwo
    ...
    # Write all text
    drawFormattedTextEx(overlay, 292, 106, 268, memo)
  end
  ...
  def drawPageThree
    ...
    # Write various bits of text
    textpos = [
      [_INTL("PS"), 366, 98, :center, base, statshadows[:HP]],
      [sprintf("%d/%d", @pokemon.hp, @pokemon.totalhp), 550, 98, :right, Color.new(64, 64, 64), Color.new(176, 176, 176)],
      [_INTL("Ataque"), 328, 140, :left, base, statshadows[:ATTACK]],
      [@pokemon.attack.to_s, 536, 140, :right, Color.new(64, 64, 64), Color.new(176, 176, 176)],
      [_INTL("Defensa"), 324, 184, :left, base, statshadows[:DEFENSE]],
      [@pokemon.defense.to_s, 536, 184, :right, Color.new(64, 64, 64), Color.new(176, 176, 176)],
      [_INTL("At. Esp"), 330, 220, :left, base, statshadows[:SPECIAL_ATTACK]],
      [@pokemon.spatk.to_s, 536, 220, :right, Color.new(64, 64, 64), Color.new(176, 176, 176)],
      [_INTL("Def. Esp"), 324, 256, :left, base, statshadows[:SPECIAL_DEFENSE]],
      [@pokemon.spdef.to_s, 536, 256, :right, Color.new(64, 64, 64), Color.new(176, 176, 176)],
      [_INTL("Velocidad"), 320, 294, :left, base, statshadows[:SPEED]],
      [@pokemon.speed.to_s, 536, 294, :right, Color.new(64, 64, 64), Color.new(176, 176, 176)],
      [_INTL("Habilidad"), 292, 340, :left, base, shadow]
    ]
    # Draw ability name and description
    ability = @pokemon.ability
    if ability
      textpos.push([ability.name, 420, 340, :left, Color.new(64, 64, 64), Color.new(176, 176, 176)])
      drawTextEx(overlay, 310, 388, 282, 2, "[ESPECIAL]: Descripción", Color.new(64, 64, 64), Color.new(176, 176, 176))
    end
    # Draw all text
    pbDrawTextPositions(overlay, textpos)
    # Draw HP bar
    if @pokemon.hp > 0
      ...
      imagepos = [
        ["Graphics/UI/Summary/overlay_hp", 650, 126, 0, hpzone * 6, w, 6]#,
  ...
  def drawPageFour
    ...
    Pokemon::MAX_MOVES.times do |i|
      ...
      if move
        ...
        imagepos.push([_INTL("Graphics/UI/types"), 320, yPos -4, 0, type_number * 28, 64, 28])
        textpos.push([move.name, 388, yPos + 2, :left, moveBase, moveShadow])
        if move.total_pp > 0
          textpos.push([_INTL("PP"), 400, yPos + 32, :left, moveBase, moveShadow])
          ppfraction = 0
          if move.pp == 0
            ppfraction = 3
          ...
          textpos.push([sprintf("%d/%d", move.pp, move.total_pp), 500, yPos + 32, :right, ppBase[ppfraction], ppShadow[ppfraction]])
        end
      else
  ...
  def drawPageFourSelecting(move_to_learn)
    ...
    textpos = [
      ...
      [_INTL("CATEGORÍA"), 10, 128, :left, base, shadow],
    ...
    limit.times do |i|
      ...
      if move
        ...
        if move.total_pp > 0
          ...
          textpos.push([sprintf("%d/%d", move.pp, move.total_pp), 500, yPos + 32, :right,
  ...
  def drawPageFive
    ...
    # Write various bits of text
    textpos = [
      [_INTL("No. de Cintas:"), 304, 390, :left, Color.new(255, 255, 255), Color.new(64, 64, 64)],
      [@pokemon.numRibbons.to_s, 474, 390, :right, Color.new(255, 255, 255), Color.new(64, 64, 64)]
    ]
    # Draw all text
    pbDrawTextPositions(overlay, textpos)
    # Show all ribbons
    ...
    loop do
      ...
      if redraw
        ...
        (@markingbitmap.bitmap.width / MARK_WIDTH).times do |i|
          markrect.x = i * MARK_WIDTH
          markrect.y = [(markings[i] || 0), mark_variants - 1].min * MARK_HEIGHT
          @sprites["markingoverlay"].bitmap.blt(310 + (58 * (i % 3)), 154 + (50 * (i / 3)),
