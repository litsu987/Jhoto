...
class PokemonPokedexInfo_Scene
  ...
  def pbDrawDataNotes(cursor = nil)
    ...
    when :ability
      pbDrawImagePositions(overlay, [[path, 310, 250, 0, 244, 116, 44]])
    ...
    when :moves
      pbDrawImagePositions(overlay, [[path, 436, 250, 0, 244, 116, 44]])
    ...
    drawFormattedTextEx(overlay, 34, 320, 446, _INTL("{1}", text))
  end
  ...
  def pbDataTextGeneral(path, species, overlay, inMenu = false)
    ...
    if owned
      ...
      pbDrawTextPositions(overlay, [
        [_INTL("Ver relacionados"), Graphics.width - 34, 316, :right, Color.new(0, 112, 248), Color.new(120, 184, 232)]
  ...
  def pbDataTextItems(path, species, overlay)
    t = DATA_TEXT_TAGS
    pbDrawImagePositions(overlay, [[path, 282, 178, 432, 208, 74, 72]])
    owned = $player.owned?(@species)
    text = ""
    if owned
      ...
      if nil_or_empty?(text)
        ...
        pbDrawTextPositions(overlay, [
          [_INTL("Más información"), Graphics.width - 34, 316, :right, Color.new(0, 112, 248), Color.new(120, 184, 232)]
        ])
      end
    else
      text << "Desconocido."
  ...
  def pbDataTextStats(path, species, overlay, s2 = nil)
    ...
    if owned
      ...
      stats_order.each_with_index do |st, i|
        ...
        nameX = 34 + 152 * i
        valueX = nameX + 100
        drawFormattedTextEx(overlay, nameX, 354, 76+16, _INTL("{1}", names))
        drawFormattedTextEx(overlay, valueX, 354, 52, _INTL("{1}", values))
      end
      pbDrawTextPositions(overlay, [
        [_INTL("Ver similares"), Graphics.width - 34, 316, :right, Color.new(0, 112, 248), Color.new(120, 184, 232)]
  ...
  def pbDataTextHabitat(path, species, overlay, s2 = nil)
    t = DATA_TEXT_TAGS
    pbDrawImagePositions(overlay, [[path, 516, 178, 432, 208, 74, 72]])
    owned = $player.owned?(@species)
    text = t[0] + "Hábitat\n"
    if owned
  ...
  def pbDataTextShape(path, species, overlay, s2 = nil)
    t = DATA_TEXT_TAGS
    pbDrawImagePositions(overlay, [[path, 438, 178, 432, 208, 74, 72]])
    text = t[0] + "Morfología\n"
    ...
    if !s2 && $player.owned?(@species) && !@data_hash[:shape].empty?
      pbDrawTextPositions(overlay, [
        [_INTL("Ver Pokémon similares"), Graphics.width - 34, 316, :right, Color.new(0, 112, 248), Color.new(120, 184, 232)]
      ])
    end
    return text
  end
  ...
  def pbDataTextEggGroup(path, species, overlay, s2 = nil)
    ...
    t = DATA_TEXT_TAGS
    pbDrawImagePositions(overlay, [[path, 356, 178, 432, 208, 74, 72]])
    owned = $player.owned?(@species)
    text = t[0] + "Crianza\n"
    if owned
      text << "Especie "
      ...
      pbDrawTextPositions(overlay, [
        [_INTL("Ver Pokémon compatibles"), Graphics.width - 34, 316, :right, Color.new(0, 112, 248), Color.new(120, 184, 232)]
  ...
  def pbDataTextFamily(path, species, overlay, inMenu = false)
    ...
    if @sprites["familyicon1"].visible && @sprites["familyicon2"].visible
      pbDrawImagePositions(overlay, [[path, 288, 90, 222, 56, 284, 76]])
    elsif @sprites["familyicon1"].visible
      pbDrawImagePositions(overlay, [[path, 340, 90, 222, 132, 180, 76]])
    else
      pbDrawImagePositions(overlay, [[path, 392, 90, 402, 132, 76, 76]])
    end
    ...
    if nil_or_empty?(text)
      ...
      if !inMenu && !@data_hash[:family].empty? #&& $player.owned?(@species)
        pbDrawTextPositions(overlay, [
          [_INTL("Ver evoluciones"), Graphics.width - 34, 316, :right, Color.new(0, 112, 248), Color.new(120, 184, 232)]
        ])
      end
    end
    return text
