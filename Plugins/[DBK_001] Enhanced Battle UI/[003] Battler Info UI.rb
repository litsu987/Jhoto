...
class Battle::Scene
  ...
  def pbUpdateBattlerInfo(battler, effects, idxEffect = 0)
    ...
    textPos  = [[_INTL("{1}", poke.name), iconX + 82, iconY - 16, :center, BASE_DARK, SHADOW_DARK],
                [_INTL("Nv. {1}", battler.level), xpos + 16, ypos + 110, :left, BASE_LIGHT, SHADOW_LIGHT],
                [_INTL("Turno {1}", @battle.turnCount + 1), Graphics.width - xpos - 32, ypos + 10, :center, BASE_LIGHT, SHADOW_LIGHT]]
    #---------------------------------------------------------------------------
    # Battler icon.
    ...
    if !battler.wild?
      ...
      textPos.push([@battle.pbGetOwnerFromBattlerIndex(battler.index).name, xpos + 32, ypos + 10, :center, BASE_LIGHT, SHADOW_LIGHT])
    end
    ...
    if battler.lastMoveUsed
      ...
      textPos.push([_INTL("Usó: {1}", movename), xpos + 348, ypos + 110, :center, BASE_LIGHT, SHADOW_LIGHT])
    end
    #---------------------------------------------------------------------------
    ...
    if battler.pbOwnedByPlayer?
      ...
      textPos.push(
        [_INTL("Hab."), xpos + 272, ypos + 48, :center, BASE_LIGHT, SHADOW_LIGHT],
        [_INTL("Obj."), xpos + 272, ypos + 72, :center, BASE_LIGHT, SHADOW_LIGHT],
        [_INTL("{1}", battler.abilityName), xpos + 376, ypos + 48, :center, BASE_DARK, SHADOW_DARK],
        [_INTL("{1}", battler.itemName), xpos + 376, ypos + 72, :center, BASE_DARK, SHADOW_DARK],
        [sprintf("%d/%d", battler.hp, battler.totalhp), iconX + 74, iconY + 16, :center, BASE_LIGHT, SHADOW_LIGHT]
      )
    end
    #---------------------------------------------------------------------------
    ...
    [[:ATTACK,          _INTL("Ataque")],
     ...
     _INTL("Críticos")
      ...
      if stat.is_a?(Array)
        ...
        textPos.push([stat[1], xpos + 17, ypos + 143 + (i * 24), :left, BASE_LIGHT, color])
        stage = battler.stages[stat[0]]
      else
        textPos.push([stat, xpos + 17, ypos + 141 + (i * 24), :left, BASE_LIGHT, SHADOW_LIGHT])
    ...
    $DELUXE_PBEFFECTS.each do |key, key_hash|
      key_hash.each do |type, effects|
        ...
        when :MiracleEye
          ...
          #---------------------------------------------------------------------
          when :Toxic
            name = _INTL("Envenenado gravemente")
          ...
          #---------------------------------------------------------------------
          when :FocusEnergy
            name = _INTL("Probabilidad de crítico aumentada")
          ...
          #---------------------------------------------------------------------
          when :Disable
            name = _INTL("Moviento Deshabilitado")
