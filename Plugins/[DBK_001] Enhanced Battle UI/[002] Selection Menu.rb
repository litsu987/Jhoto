    ...
    2.times do |side|
      ...
      when 0
        @battle.allSameSideBattlers.each_with_index do |b, i|
          case count
          when 1 then iconX, bgX = 266, 173
          when 2 then iconX, bgX = 216 + (208 * i), 68 + (208 * i)
          when 3 then iconX, bgX = 82 + (168 * i), 4 + (169 * i)
          end
          iconY = ypos + 114
          nameX = iconX + 80
          if idxSide == side && idxPoke == i
            ...
            imagePos.push([@path + "select_cursor", bgX + 66, iconY - 28, 0, 52, 166, 52])
          else
            base, shadow = BASE_DARK, SHADOW_DARK
            imagePos.push([@path + "select_cursor", bgX + 66, iconY - 28, 0, 0, 166, 52])
          end
          @sprites["info_icon#{b.index}"].x = iconX
          @sprites["info_icon#{b.index}"].y = iconY
          pbSetWithOutline("info_icon#{b.index}", [iconX, iconY, 300])
          imagePos.push([@path + "info_owner", bgX + 86, iconY + 11],
                        [@path + "info_gender", bgX + 220, iconY - 37, b.gender * 22, 0, 22, 20])
          textPos.push([_INTL("{1}", b.pokemon.name), nameX - 10, iconY - 16, :center, base, shadow],
                       [@battle.pbGetOwnerFromBattlerIndex(b.index).name, nameX - 20, iconY + 15, 2, BASE_LIGHT, SHADOW_LIGHT])
        end
      ...
      when 1
        @battle.allOtherSideBattlers.reverse.each_with_index do |b, i|
          ...
          imagePos.push([@path + "info_gender", bgX + 220, iconY - 37, b.displayPokemon.gender * 22, 0, 22, 20])
        end
        if @battle.opponent
