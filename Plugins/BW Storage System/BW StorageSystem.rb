  ...
  def refresh
    if @refreshBox
      ...
      pbDrawShadowText(@contents, xval, 14, widthval, 52,
                       boxname, Color.new(255, 255, 255), Color.new(132, 132, 132))
      @refreshBox = false
    end
...
class PokemonBoxPartySprite < Sprite
  ...
  def refresh
    ...
    pbDrawTextPositions(
      ...
      [[_INTL("Atras"), 86, 248, :center, Color.new(239, 239, 239), Color.new(132, 132, 132)]]
    )
...
class PokemonStorageScene
  ...
  def pbSwitchBoxToRight(new_box_number)
    ...
    newbox.x = start_x + 342
    timer_start = System.uptime
    loop do
      @sprites["box"].x = lerp(start_x, start_x - 342, 0.25, timer_start, System.uptime)
      newbox.x = @sprites["box"].x + 342
      self.update
      Graphics.update
    ...
    loop do
      ...
      if redraw
        ...
        textpos = [
          ...
          [_INTL("OK"), 402, 212, :center, Color.new(239, 239, 239), Color.new(132, 132, 132), :outline],
          [_INTL("Cancel"), 402, 276, :center, Color.new(239, 239, 239), Color.new(132, 132, 132), :outline]
        ]
  ...
  def pbUpdateOverlay(selection, party = nil)
    ...
    if !pokemon.egg?
      ...
      # Changed Text Positions
      if pokemon.male?
        textstrings.push([_INTL("♂"), 148, 18, :left, Color.new(0, 0, 214), Color.new(15, 148, 255)])
      elsif pokemon.female?
        textstrings.push([_INTL("♀"), 148, 18, :left, Color.new(198, 0, 0), Color.new(255, 155, 155)])
      end
      imagepos.push(["Graphics/UI/Storage/overlay_lv", 6, 320])
      textstrings.push([pokemon.level.to_s, 28, 320, :left, Color.new(255, 255, 255), Color.new(90, 82, 82)])
      if pokemon.ability
        textstrings.push([pokemon.ability.name, 16, 388, :left, base, shadow])
      else
        textstrings.push([_INTL("Sin habilidad"), 16, 358, :left, nonbase, nonshadow])
      end
      if pokemon.item
        textstrings.push([pokemon.item.name, 16, 418, :left, base, shadow])
      else
        textstrings.push([_INTL("Sin objeto"), 16, 418, :left, nonbase, nonshadow])
      end
      # Changed Shiny Icon
