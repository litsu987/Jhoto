...
class PokemonSummary_Scene
  ...
  def drawPage(page)
    ...
    imagepos.push([ballimage, 14, 70])
    pagename = UIHandlers.get_info(:summary, @page_id, :name)
    textpos = [
      [pagename, 22, 26, :left, base, shadow],
      [@pokemon.name, 46, 82, :left, base, shadow],
      [_INTL("Objeto"), 122, 374, :left, base, shadow]
    ]
    if @pokemon.hasItem?
      textpos.push([@pokemon.item.name, 12, 412, :left, Color.new(64, 64, 64), Color.new(176, 176, 176)])
    else
      textpos.push([_INTL("Ninguno"), 12, 412, :left, Color.new(64, 64, 64), Color.new(208, 216, 224)])
    end
    ...
    if !@pokemon.egg?
      ...
      if status >= 0
        imagepos.push(["Graphics/UI/statuses", 150, 109, 0, 16 * status, 44, 16])
      end
      if @pokemon.pokerusStage == 2
        imagepos.push(["Graphics/UI/Summary/icon_pokerus", 176, 100])
      end
      imagepos.push(["Graphics/UI/shiny", 2, 134]) if @pokemon.shiny?
      textpos.push([@pokemon.level.to_s, 56, 120, :left, Color.new(255, 255, 255), Color.new(62, 62, 62)])
      if @pokemon.male?
        textpos.push([_INTL("♂"), 6, 118, :left, Color.new(24, 112, 216), Color.new(136, 168, 208)])
      elsif @pokemon.female?
        textpos.push([_INTL("♀"), 6, 118, :left, Color.new(248, 56, 32), Color.new(224, 152, 144)])
      end
    end
    # Draws the page.
